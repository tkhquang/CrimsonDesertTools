...
namespace CDCore
{
     ...
     *          directly, reaching the same controlled CCOIA the CDCore chain resolves indirectly through the
     *          sub-manager (+0x38).
     *
     *          A game update that re-lays-out pa::ClientActorManager moves k_actorManagerToUserActor; it is the offset
     *          most likely to change on a patch, which is why it lives here once rather than inline at each call site.
     ...
     *          decoy-free across the whole heal range. Raise it via the INI only if a real patch shifts the field
     *          further.
     */
     ...
     *          guarded. Returns 0 when the chain is mid-teardown (engine published a null along the path during
     *          save-load) or before the engine has finished cold-load wiring.
     */
    [[nodiscard]] std::uintptr_t current_controlled_ccoia() noexcept;

    /**
     * @brief Resolve the currently-controlled character.
     * @details Walks the static chain to obtain the controlled CCOIA, then classifies it by reading the
     *          appearance-config asset path at `CCOIA + 0x68 -> +0x40 -> +0x40 -> +0x38` and matching the embedded
     *          character codename (`cd_phm_macduff` / `cd_phw_damian` / `cd_phm_oongka`).
     *
     *          Falls back to the `+0x63` high-byte fast-path for
     *          Kliff when the appearance chain is mid-teardown (an NPC-controlled cutscene frame or save-load window
     ...
     * @details Walks `*(ccoia + 0x68) + 0x38`, which the engine pins to `pa::ClientEquipSlotActorComponent`.
     *          SEH-guarded; returns 0 on any null/fault or on inactive bodies where the engine zeroes the component
     *          slot.
     *
     *          Useful for iterating non-controlled player CCOIAs and dispatching engine APIs that take an equip-slot a1
     ...
     *          @ref current_controlled_character_idx uses: 1 for Kliff, 2 for Damiane, 3 for Oongka. Returns 0 for
     *          paths that don't contain a known codename (NPCs, monsters, empty paths).
     *
     *          Exposed so callers that already hold an actor's appearance / asset string (read from any source, not
     *          necessarily the CCOIA chain) can classify it without having to re-derive the chain walk used by @ref
     *          current_controlled_character_idx.
     *
     ...
     *            - Damiane / Oongka are pulled from the `ClientActorManager + 0x130` CCOIA-only actor array (8-byte
     *              stride) and accepted only when the appearance-config classifier matches their character codename.
     *              NPCs and follower humanoids share the array but fail the codename match, so the snapshot returns at
     *              most one Damiane and one
     *              Oongka entry.
     *
     *          The walk is SEH-guarded; on any fault returns 0. Kliff is always written first; remaining entries are in
     ...
     *          Kliff CCOIA pointer at sub-manager +0x30. The CCOIA sub-manager pointer itself is persistent across
     *          save-load (its address never changes within a process lifetime), so it cannot be used as the
     *          world-rebuild signal; Kliff's CCOIA IS reallocated on every save-load and is the correct trigger.
     *          Consumers cache the previous value and treat any change as "world has been rebuilt, flush local
     *          per-world state".
     *
     ...
     *          entry, without filtering by character classifier. Use to diagnose snapshot misses (e.g., when a
     *          protagonist's +0x60 load-order delta does not match the expected
     ...
     *          auto-reload). Empty strings are ignored -- the corresponding codename keeps its current value. Built-in
     *          defaults:
     ...
     *          Cheap; safe from any thread. Call on world-reload transitions if you want a defensively-fresh generation
     *          bump (the regular Kliff-CCOIA delta tracking already covers normal save-loads, so this is rarely
     *          required).
     */
     ...
     *        name.
     * @details Component slots in @p p1 are sparse and slot-drift across early/late load (a class can sit at +0x40 in
     *          steady state and +0x48 during cold load). Hardcoded slot offsets are fragile; this scanner identifies
     *          the target by traversing
     *          each non-null slot's vtable -> RTTICompleteObjectLocator
     *          -> TypeDescriptor and exact-comparing the mangled name.
     *
     *          The vtable address of the first match is cached via the caller-supplied @p vtableCache so subsequent
     *          calls fast-path on a single qword compare per slot. The cache lives for process lifetime (image-resident
     *          vtables); the caller declares one `std::atomic<std::uintptr_t>` static per component class.
     *
     *          SEH-guarded. Returns 0 on torn reads, missing class, or when @p p1 / @p rttiName is invalid.
     *
     ...
     *          engine offsets. SEH-guarded end-to-end. Used by readiness probes that need to inspect a specific sibling
     *          component of any protagonist (not just the controlled character).
