...
//
// Each candidate is uniqueness-verified against the live .text section at authoring time; cascades exist so partial
// recompilations or sibling-DLL hook patches that invalidate the tightest signature still resolve.
//
...
//     renumber.
//   - Keep signatures as short as will return a unique hit (7-16 bytes is the common sweet spot; 12-32 bytes when
//     disambiguation needs it).
...
namespace CDCore::Anchors
{
    ...
    // actor component. RipRelative mode:
    ...
    inline constexpr AddrCandidate k_worldSystemCandidates[] = {
        // P1 -- whole-function anchor for the small getter (the tightest signature we can build: it pins every byte of
        // the function prologue AND body).
        ...
        //
        // The 2-byte branch slot is wildcarded rather than hard-coded as `75 ??`: per aob-signatures.md section 8 the
        // compiler may flip a short
        // Jcc to the 6-byte `0F 85 rel32` form, which would change the opcode byte. Wildcarding both bytes tolerates
        // the 2-byte shape regardless of opcode, but the pattern still fails on a 6-byte flip -- in that case WS_P1 /
        // WS_P3 take over (they don't cross a branch). The trailing `48 8B 88 D8 00 00 00` pins the specific
        // WorldSystem follow-on (`mov rcx, [rax+0xD8]`); 0xD8 is a game-struct ABI offset, stable within a build.
    ...
    inline constexpr AddrCandidate k_mapLookupCandidates[] = {
        ...
        // distinctive. The 2-byte early-out branch slot is wildcarded (see section 8 branch-encoding note in the
        // WorldSystem
        // P2 comment above).
        ...
        // walks back to func start. The 2-byte jz on zero-count is wildcarded (same branch-encoding caveat as above).
        {"MapLookup_P2_HashBody", "8B 48 ?? 48 03 D2 44 8B 5C D1 ?? 41 8B 08 85 C9 ?? ?? 33 D2 41 8B C3 F7 F1",
         ResolveMode::Direct, -0x24, 0},

        // P3 -- hash-loop anchor (even deeper). Last-resort fallback; offset -0x3D walks back to func start.
    ...
    // -----------------------------------------------------------------------
    // VisualEquipChange (sub_14076D520) -- bottleneck for all visual equipment changes (equip and unequip). Called from
    // the network handler for TrocTrAddVisualEquipItemAck.
    //
    // Signature (x64 __fastcall):
    ...
    // P1s). P2 is a "push-frame" anchor unique to the function's specific prologue shape. P3 is the post-alloca
    // register-shuffle anchor. P4 is a deeper body anchor past the `lea rcx,[rbp+X]`.
    // -----------------------------------------------------------------------
    inline constexpr AddrCandidate k_visualEquipChangeCandidates[] = {
        ...
        // register moves). The wildcarded stack-size and function-size slots alone match 5 unrelated prologues; adding
        // the `48 2B E0 49 8B F1 41 0F B7 D8` tail (specific VEC register shuffle through `movzx ebx, r8w`) restores
        ...
        // movzx ebx,r8w; movzx edi,dx; mov r14,rcx) + deeper `lea rcx, [rbp+disp32]` + `E8` call. Stack disp32
        // wildcarded per section 2. Offset -0x2A backs up to function start.
    ...
    // -----------------------------------------------------------------------
    // BatchEquip (sub_14075BBF0) -- the function that fires when the player equips armor. Reads the entry table at
    // a1+120, builds 216-byte dispatch entries, and calls EquipChangeDispatch (sub_14075C420) to load meshes. Also the
    // bottleneck for item-to-item visual swaps (bypasses VisualEquipChange). Named BatchEquip here; the EquipHide
    // cascade code calls the same role VisualEquipSwap.
    //
    // Signature (x64 __fastcall):
    ...
    // scratch zero-init.
    //
    // The engine's arg shuffle assigns a1 to r14 (a naive prologue would expect rsi: `48 8B F1` -> `4C 8B F1`) and
    // reads [a1+8] into r13 (was r14: `4C 8B 71 08` -> `4C 8B 69 08`). Instruction lengths are unchanged, so the -0x22
    // / -0x25 walk-backs still resolve to the function start. Frame/function-size immediates stay wildcarded per
    // section 2.
    // -----------------------------------------------------------------------
    inline constexpr AddrCandidate k_batchEquipCandidates[] = {
        ...
        // rsp,rax, first 3 post-alloca register moves, extending into the body shuffle -- the wildcarded stack-size and
        // function-size slots alone match 5 unrelated prologues; the `48 2B E0 4D 8B F8 4C 8B E2 4C 8B F1 4C 8B 69 08`
        // tail restores uniqueness without a hardcoded stack frame (section 2).
        ...
        // mov r12,rdx; mov r14,rcx; mov r13,[rcx+8]) + the on-stack scratch prep (lea rax,[rbp+X]; mov [rbp+Y],rax; xor
        // ecx,ecx). Stack disp8 offsets wildcarded per section 2. Offset -0x22 backs up to function start.
        //
        ...
        // scratch zero-init (mov [rbp+X],ecx; mov dword [rbp+Y], 2). Stack disp8 offsets wildcarded; the `02 00 00 00`
        // tail is SEMANTIC (the value-2 init flag), kept literal per section 2 exception for semantic constants. Offset
        // -0x25 backs up to function start.
    ...
    // controlled_char.cpp.
    //
    // The slot's module-relative offset drifts between game patches (e.g., 0x5FA0430 on v1.06 / v1.07, 0x6012110 on
    // v1.08), so a hardcoded offset silently reads unrelated `.data` on the wrong build (the v1.06 offset on a v1.08
    // binary lands inside a packed string table, dereferencing to ASCII content rather than a heap pointer). All three
    // candidates anchor on distinct instructions inside the same lazy-init function that publishes the singleton and
    // resolve to the same global slot.
    //
    ...
    inline constexpr AddrCandidate k_clientActorManagerGlobalCandidates[] = {
        ...
        // function. disp32 of `mov [rip+disp32], rdi` is at match+3; the instruction is 7 bytes long.
        ...
        // vmovdqu resolves to the same global slot as P1. disp32 lives at match+8; vmovdqu instruction ends at
        // match+12.
        ...
        // leading `lea rdx; lea rcx,[rbp+]; call; nop` shape matched ten sites in v1.08. disp32 of the `lea rdx` lives
        // at match+3; the instruction is 7 bytes long.
