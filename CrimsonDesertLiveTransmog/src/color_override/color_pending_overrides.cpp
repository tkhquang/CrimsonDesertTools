...
namespace Transmog::ColorOverride::PendingOverrides
{
             ...
             std::uint8_t b) noexcept
    {
        if (!valid_slot(slot))
            return;
        if (submesh_name.empty() || token_name.empty())
            return;
                ...
                std::uint8_t &b) noexcept
    {
        ...
        if (!g_slotHas[static_cast<std::size_t>(slot)].load(std::memory_order_acquire))
        {
            g_misses.fetch_add(1, std::memory_order_relaxed);
            return false;
        }
        ...
        for (auto it = vec.begin(); it != vec.end(); ++it)
        {
            ...
            vec.erase(it);
            if (vec.empty())
                g_slotHas[static_cast<std::size_t>(slot)].store(false, std::memory_order_release);
    ...
    bool slot_has_pending(int slot) noexcept
    {
        if (!valid_slot(slot))
            return false;
        return g_slotHas[static_cast<std::size_t>(slot)].load(std::memory_order_acquire);
    }
                         ...
                         std::uint8_t g, std::uint8_t b) noexcept
    {
        if (!valid_slot(slot))
            return;
        if (submesh_name.empty() || token_id == 0)
            return;
        std::lock_guard<std::mutex> lk(g_mtx);
        auto &vec = g_entries[static_cast<std::size_t>(slot)];
        ...
        for (auto &e : vec)
        {
            if (e.token_id_cached == token_id && e.submesh_name == submesh_name)
            {
                e.r = r;
                e.g = g;
                e.b = b;
    ...
    void erase_by_token_id(int slot, const char *submesh_name, std::uint16_t token_id) noexcept
    {
        ...
        for (auto it = vec.begin(); it != vec.end(); ++it)
        {
            if (it->token_id_cached == token_id && it->submesh_name == submesh_name)
            {
                vec.erase(it);
                if (vec.empty())
                    g_slotHas[static_cast<std::size_t>(slot)].store(false, std::memory_order_release);
