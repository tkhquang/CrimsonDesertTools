...
namespace Transmog::ColorOverride::HostScope
{
    namespace
    {
        // Per-host owner-container vfuncs. Each is a vtable slot on
        // the per-host matInst-container; at entry RCX IS the owner
        // container (no `[+0x10]` deref). They internally call
        // sub_141026640's matInst-list copy loop, which dispatches the
        // publisher, which eventually invokes the 4-byte property
        // setter that ColorOverride::SetterSubstitute hooks. Both
        // targets are resolved live via the AOB cascade in
        // `aob_resolver.hpp` so they survive function relocation
        // across patches.

        // Per-thread RSP-tagged owner state. x64 RSP grows DOWNWARD,
        // so setter_rsp < tl_ownerRsp means setter is inside the iter
        // (live owner); setter_rsp >= tl_ownerRsp means iter has
        // returned (stale -- gate falls back to permissive since the
        // setter's call frame isn't covered by an iter at all).
        thread_local std::uintptr_t tl_ownerParent = 0;
        thread_local std::uintptr_t tl_ownerRsp = 0;

        // Cluster histogram: bounded lock-free open-address table.
        // Writes are from the engine render thread only, so the
        // atomics handle the rare publisher-side reader cleanly.
        struct ClusterEntry
        {
        ...
        bool looks_like_live_host(std::uintptr_t parent) noexcept
        {
            if (parent == 0)
                return false;
            // Heap range sanity: container parents land in 0x4xxxxxxxxx
            // on this engine. Anything below the 4 GiB boundary is
        ...
        void cluster_record(std::uintptr_t parent) noexcept
        {
            ...
            for (auto &e : g_cluster)
            {
                ...
                if (cur == 0)
                {
                    ...
                    if (e.parent.compare_exchange_strong(
        ...
        // True when the cluster has enough hits for an election to
        // be meaningful. Below floor the gate stays permissive so the
        // first ~5 vfunc calls of an apply don't lock the substitute
        // out before the cluster has any data.
        ...
        // `hits >= top * 0.1`. The 10 % threshold admits dual-parent
        // player + LOD-cull variance while still cleanly rejecting
        // NPCs whose hits sit below ~1 % of the leader.
        ...
        // Mid-hook on the per-host owner-container vfuncs. RCX at
        // entry IS the owner container; the iter that dispatches the
        // publisher (and ultimately the setter) lives below this
        // frame on the stack, so the setter's RSP < tl_ownerRsp for
        // live captures.
    ...
    {
        // Cold start: no iter has ever pushed an owner. Permissive
        // so DLL-load and pre-first-apply renders aren't dropped.
        if (tl_ownerRsp == 0)
            return true;

        // The iter has returned: the setter call frame is no longer
        // nested inside any owner-vfunc, so we have no host scope
        // to gate by. Permissive in this case (stale -> permissive
        // is the documented outcome) and counted separately for
        // visibility.
        ...
        // Inside an iter with a live host, but the cluster hasn't
        // built up enough signal yet. Permissive so we don't reject
        // legitimate first-apply writes that fire before any NPC
        // host has been seen.
        if (!election_ready())
            return true;
    ...
    bool init()
    {
        ...
        const auto addr1 = ::Transmog::resolve_address(
            ::Transmog::k_hostScopeVfunc1Candidates,
            "HostScopeVfunc1");
        const auto addr2 = ::Transmog::resolve_address(
            ::Transmog::k_hostScopeVfunc2Candidates,
            "HostScopeVfunc2");
        if (addr1 == 0 && addr2 == 0)
            return false;
        ...
        if (addr1 != 0)
        {
            auto r1 = hookMgr.create_mid_hook(
                "HostScopeVfunc1", addr1, &on_iter_entry);
            if (!r1.has_value())
            {
                log.warning(
                    "[dye-host-scope] vfunc1 hook FAILED at {:#x}: {}",
                    addr1,
        ...
        if (addr2 != 0)
        {
            auto r2 = hookMgr.create_mid_hook(
                "HostScopeVfunc2", addr2, &on_iter_entry);
            if (!r2.has_value())
            {
                log.warning(
                    "[dye-host-scope] vfunc2 hook FAILED at {:#x}: {}",
                    addr2,
