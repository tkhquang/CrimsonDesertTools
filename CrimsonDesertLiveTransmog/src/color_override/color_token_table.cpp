...
namespace Transmog::ColorOverride::TokenTable
{
    namespace
    {
        // `_permutations` is resolved independently of the dye-token discovery path: the publisher hook reads it live
        // from a known matInst and calls set_permutations_token() on its first fire. Default 0 (unresolved) until then.
    ...
    namespace
    {
        // Classify by property NAME prefix. Returns layer (0..3) or -1. Channel inferred from R/G/B suffix.
        int classify_by_name(const char *name, int *out_channel) noexcept
        {
            ...
            const int ch = (last == 'R') ? 0 : (last == 'G') ? 1 : (last == 'B') ? 2 : -1;
            if (out_channel)
                *out_channel = ch;
            ...
            //
            // Visually distinct from the main armor tint: the scratch layer is the wear/damage overlay color. Keeping
            // it separate from `_tintColor*` (layer 0) lets users pick these independently rather than always coupling
            // them.
            if (starts("_scratchTintColor"))
                return 4;
            // _tintColor* -> tint (0)
            if (starts("_tintColor"))
                return 0;
    ...
    const char *layer_long_name(int layer) noexcept
    {
        ...
        case 0:
            return "_tintColor";
        case 1:
            return "_dyeingColorMask";
        case 2:
            return "_dyeingDetailLayerColorMask";
        case 3:
            return "_hairDyeingColor";
        case 4:
            return "_scratchTintColor";
        default:
            return "misc";
        }
    }
    ...
    const char *channel_short_name(int channel) noexcept
    {
        ...
        case 2:
            return "B";
        default:
            return "\xC2\xB7"; // U+00B7 middle dot
        }
    }
