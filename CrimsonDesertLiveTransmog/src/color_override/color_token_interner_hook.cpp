    ...
    namespace
    {
                                                   ...
                                                   std::size_t modSize) noexcept
        {
            __try
            {
                ...
                for (std::size_t i = 0; i + 7 <= k_scanLen; ++i)
                {
                    ...
                    if ((b0 == 0x48 || b0 == 0x4C) && b1 == 0x89 && (b2 & 0xC7) == 0x05)
                    {
                        ...
                        const auto target = funcAddr + i + 7 + static_cast<std::intptr_t>(disp);
                        if (target < modBase || target >= modBase + modSize)
                            continue;
        ...
        // Walk the interner's entries array and capture every (name, token) pair from `startIdx` onward. Returns the
        // number of entries captured into the global table; on return `nextIdx` is set to the array index where the
        ...
        std::size_t walk_entries_array(std::uintptr_t entriesBase, std::size_t startIdx, std::size_t &nextIdx) noexcept
        {
            ...
            for (; i < k_maxWalk; ++i)
            {
                ...
                consecutiveBad = 0;
                const auto *name = reinterpret_cast<const char *>(namePtr);
            ...
            // `consecutiveBad` is the number of consecutive misses we saw before bailing; the first "real" entry sits
            // at `i - consecutiveBad`, so resume at that index.
        ...
        if (stateAddr >= 0x10000ULL)
        {
            ...
            if (entriesBase >= 0x10000ULL)
            {
                ...
                if (cachedBase == entriesBase)
                    startIdx = g_lastEntriesIdx.load(std::memory_order_acquire);
