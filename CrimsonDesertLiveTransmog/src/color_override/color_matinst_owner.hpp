...
// (cheap pool-recycle detection). The setter hook reads this map to
// resolve which LT slot a setter event belongs to without depending
// on the apply-window timer.
...
namespace Transmog::ColorOverride::MatInstOwner
{
    ...
    /// Look up the slot for `mi` only if its expected_hash matches
    /// `live_hash`. Returns -1 on miss or hash mismatch (pool
    /// recycle).
