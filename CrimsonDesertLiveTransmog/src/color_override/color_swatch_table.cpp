...
namespace Transmog::ColorOverride
{
    ...
    namespace { bool g_dyeAdvancedView = false; }

    bool dye_advanced_view_get() noexcept { return g_dyeAdvancedView; }
    void dye_advanced_view_set(bool v) noexcept { g_dyeAdvancedView = v; }
}
...
namespace Transmog::ColorOverride::SwatchTable
{
    namespace
    {
        ...
        std::array<std::array<SwatchEntry, k_dyeSwatchesPerSlot>, k_slotCount>
            g_table{};
    ...
    {
        return lookup_or_insert(slot, content_hash, stable_id,
                                template_id, token_id, expect_open,
                                nullptr);
    }
                         ...
                         const char *submesh_name) noexcept
    {
        ...
        {
            auto &row = table[i];
            // Match the stable-identity tuple
            // (stable_id, template_id, token_id) before validating
            // content_hash -- the stable triple is the strict
            // identity, so most mismatches bail without touching
            // the volatile hash.
            if (row.stable_id.load(std::memory_order_relaxed) != stable_id
                || row.template_id.load(std::memory_order_relaxed)
                       != template_id
                || row.token_id.load(std::memory_order_relaxed) != token_id)
                continue;
            if (row.content_hash.load(std::memory_order_relaxed)
                != content_hash)
                continue;
            // Ghost guard: refuse to resurrect rows that
            // Reinit::Finalize froze out. Returning -1 makes the
            // setter bail without substituting AND without re-
            // marking active_this_apply = true, so the next reinit
            // pass's intersection sees a "miss" on this identity.
        ...
        //
        // `populate_from_persisted` creates rows with the saved
        // (submesh_name, token_id) but content_hash=0 to mark them as
        // "awaiting promotion". The first engine write whose
        // (submesh_name, token_id) matches a placeholder gets that
        // row's hash/stable/tpl filled in -- subsequent writes hit
        // the fast hash-based loop above. This is how saved presets
        // re-bind to live engine identities without running the
        // 3-pass Reinit capture.
        ...
        if (State::swatch_frozen(slot).load(std::memory_order_acquire))
            return -1;
        // Reinit post-lock: once Finalize closes the slot's identity
        // set, refuse new inserts until the user explicitly re-runs
        // reinit / Reset slot / preset switch (which clears the lock).
        if (g_postReinitLock[static_cast<std::size_t>(slot)].load(
                std::memory_order_acquire))
        {
            // While the reinit capture window is open (during a
            // re-run), allow inserts again so passes 2+3 can refresh
            // the identity set.
            if (!g_reinitCaptureOpen[static_cast<std::size_t>(slot)].load(
                    std::memory_order_acquire))
                return -1;
        }
        ...
        row.active_this_apply.store(true, std::memory_order_release);
        // Mirror row identity to the UI-side SwatchOverride so the
        // picker (which reads `dyeSlot.swatches[]` directly) can
        // render labels and region grouping without joining
        // against SwatchEntry.
        {
            ...
            ovr.submesh_stable_id = stable_id;
            ovr.template_id = template_id;
            // Seed submesh_name once on insert. Re-fires for the same
            // identity tuple don't re-write (see lookup branch above)
            // so a transient null capture doesn't clobber a good
            // earlier read.
            if (submesh_name && submesh_name[0] != '\0')
            {
                std::size_t n = 0;
                while (n < sizeof(ovr.submesh_name) - 1
                       && submesh_name[n] != '\0')
                {
    ...
    std::size_t count(int slot) noexcept
    {
        if (!valid_slot(slot))
            return 0;
        const auto raw =
            g_count[static_cast<std::size_t>(slot)].load(std::memory_order_acquire);
                                  ...
                                  std::uint8_t b, std::uint8_t a) noexcept
    {
        ...
        if ((mask & 1u) == 0)
        {
            ...
            ovr.def_g = g;
            ovr.def_b = b;
            // Seed the user override with the engine default ONLY
            // when the row isn't already user-overridden, otherwise
            // the setter's first capture on a saved or loaded row
            // would clobber the user's picked colour. Capture
            // itself ALWAYS runs so the picker's "default colour
            // reference dot" has live def_r/g/b for every row,
            ...
            row.def_seen_mask.store(static_cast<std::uint8_t>(mask | 1u),
    ...
    bool any_override_active_in_slot(int slot) noexcept
    {
        ...
        const auto upper =
            (cnt < k_dyeSwatchesPerSlot) ? cnt : k_dyeSwatchesPerSlot;
        for (std::size_t i = 0; i < upper; ++i)
            if (dye_state()[s].swatches[i].override_active) return true;
        return false;
    }
    ...
    void clear_slot(int slot) noexcept
    {
        ...
        // Preserve user override RGB across wipes so re-tick of the
        // same item restores the user's picks; only kill the
        ...
        for (auto &o : dye_state()[s].swatches)
        {
            ...
            // Identity-bound: same item re-applies will re-capture
            // the name on insert.
            o.submesh_name[0] = '\0';
        }
    ...
    void clear_all() noexcept
    {
        ...
        // clear_slot preserves override choices; wipe them too on a
        // global reset (preset/character switch).
    ...
    void wipe_swatch_table_for_slot(int slot) noexcept
    {
        ...
        g_count[s].store(0, std::memory_order_release);
        // Wipe leaves the slot LOCKED. Inserts only happen inside an
        // explicit Reinit capture window (user-clicked Re-init or
        // auto-triggered by preset switch). Without an open window,
        // setter writes only MATCH existing rows; a missing identity
        // falls through (the engine renders its natural value, no
        // new row is added), keeping stray captures from polluting
    ...
    std::atomic<bool> &post_reinit_lock(int slot) noexcept
    {
        ...
        if (!valid_slot(slot)) return dummy;
    ...
    // Helper: shared row-identity gate used by both getters.
    // Returns the token_name for valid rows, or nullptr to skip.
    static const char *resolve_persist_token_name(
        const SwatchEntry &e,
        const SwatchOverride &ovr) noexcept
    {
        if (ovr.submesh_name[0] == '\0') return nullptr;
    ...
    std::vector<PersistEntry> get_persistable_overrides(int slot) noexcept
    {
        ...
        // Diagnostic: count + categorise every drop so we can see
        // exactly why some rows aren't persisting.
        ...
        for (std::size_t i = 0; i < upper; ++i)
        {
            ...
            if (tokenId == 0)
            {
                ...
                logger.debug(
                    ...
                    "{:02X},{:02X})",
                    slot, i, ovr.submesh_name, ovr.r, ovr.g,
                    ovr.b);
                continue;
            }
            const char *tokenName = TokenTable::token_label_for(
                static_cast<std::uint16_t>(tokenId));
            if (tokenName == nullptr)
            {
                ...
                logger.debug(
                    ...
                    "{:02X},{:02X})",
                    slot, i, tokenId, ovr.submesh_name,
        ...
        logger.debug(
            ...
            "drop_no_token_label={}",
            slot, total, kept, drop_inactive, drop_no_submesh,
            drop_no_token_id, drop_no_token_label);
    ...
    std::vector<PersistEntry> get_persistable_palette(int slot) noexcept
    {
        ...
        for (std::size_t i = 0; i < upper; ++i)
        {
            const auto &e   = g_table[s][i];
            const auto &ovr = dye_state()[s].swatches[i];
            // Identity-only filter: any row whose submesh+token is
            // resolvable gets recorded in the palette, regardless of
            // whether it's user-overridden or has a captured default.
            // r/g/b is left zero -- callers shouldn't read them; the
            // palette JSON serializer writes just the token names.
            const char *tokenName = resolve_persist_token_name(e, ovr);
            if (tokenName == nullptr) continue;
            PersistEntry pe{};
            pe.submesh_name = ovr.submesh_name;
            pe.token_name   = tokenName;
            out.push_back(std::move(pe));
    ...
    void restore_persisted_state(
        int slot, const std::vector<PersistEntry> &entries) noexcept
    {
        if (!::Transmog::flag_color_override().load(
                std::memory_order_acquire))
            return;
        ...
        for (const auto &pe : entries)
        {
            ...
            // Always queue into the pending-overrides map; the setter
            // consults this on every successful lookup_or_insert so a
            // freshly-captured row gets its persisted color the moment
            // the engine writes to it.
            PendingOverrides::set(slot, pe.submesh_name, pe.token_name,
                                  pe.r, pe.g, pe.b);
            ++queued;

            // Plus an opportunistic immediate-apply pass: if a row
            // matching this (submesh, token) is ALREADY in the live
            // swatch table (e.g. we're restoring after a preset
            // switch where the same outfit is already applied), set
            // its override RGB right now so the picker reflects the
            // new color without waiting for the next engine write.
            const auto tok = TokenTable::token_id_for_name(
                pe.token_name.c_str());
            if (tok == 0) continue;
            for (std::size_t i = 0; i < upper; ++i)
            {
                ...
                if (row.token_id.load(std::memory_order_relaxed) != tok)
                    continue;
                auto &ovr = dye_state()[s].swatches[i];
                if (std::strcmp(ovr.submesh_name,
                                pe.submesh_name.c_str()) != 0)
                    continue;
                ovr.r = pe.r;
                ovr.g = pe.g;
                ovr.b = pe.b;
    ...
    {
        if (token_id == 0 || submesh_name == nullptr
            || submesh_name[0] == '\0')
            return -1;
        for (std::size_t s = 0; s < k_slotCount; ++s)
        {
            const auto cnt = g_count[s].load(std::memory_order_acquire);
            const auto upper = (cnt < k_dyeSwatchesPerSlot)
                ? cnt : k_dyeSwatchesPerSlot;
            for (std::size_t i = 0; i < upper; ++i)
            {
                const auto &row = g_table[s][i];
                // Placeholders are rows seeded by
                // populate_from_persisted: content_hash==0 marks
                // "awaiting promotion". Live rows already match by
                // hash in the substitute path's normal flow.
                if (row.content_hash.load(std::memory_order_relaxed) != 0)
                    continue;
                if (row.token_id.load(std::memory_order_relaxed) != token_id)
                    continue;
        ...
        std::uint8_t def_a) noexcept
    {
        ...
        for (std::size_t s = 0; s < k_slotCount; ++s)
        {
            ...
            for (std::size_t i = 0; i < upper; ++i)
            {
                ...
                if (std::strcmp(ovr.submesh_name, submesh_name) != 0)
                    continue;
                // Promote: write live identity fields into the
                // SwatchEntry. template_id / stable_id first, then
                // content_hash last (release-ordered) so any reader
                // that sees a non-zero content_hash is guaranteed
                // to also see the other identity fields.
                row.template_id.store(template_id,
                                      std::memory_order_relaxed);
                row.stable_id.store(stable_id,
                                    std::memory_order_relaxed);
                row.content_hash.store(content_hash,
                                       std::memory_order_release);
                // Mirror template_id into the picker-facing
                // SwatchOverride too. The UI reads `sw.template_id`
                // from SwatchOverride (not SwatchEntry) when building
                // the region-header label "submesh  (tpl 0x%04X)" --
                // without this mirror the label stays at 0x0000 even
                // though SwatchEntry's template_id is correct.
                // submesh_stable_id is already populated by
                // populate_from_persisted at JSON load (FNV of
                // submesh_name) so the UI's region grouping works
                // before promotion; we leave it as-is.
                ovr.template_id = template_id;
                // Capture the engine's intended source RGB (passed
                // in by the setter from ctx.r8 BEFORE the substitute
                // redirected the pointer) as this row's default.
                // Without this the picker never shows a default-
                // colour reference dot for placeholder rows, because
                // the slot-agnostic pending-match path returns
                // before the setter's normal capture path runs.
                // capture_default_if_unset is a no-op once a default
                // is captured, so this is idempotent across repeated
                // engine writes.
        ...
        const std::vector<PersistEntry> &overrides) noexcept
    {
        ...
        // FNV-1a 64-bit hash for synthetic stable_id assignment to
        // placeholders. Distinct per submesh so the UI's region
        // grouping map produces one bucket per submesh. Promotion in
        // lookup_or_insert later overwrites with the engine's real
        // pool counter when an identity match fires.
        ...
        // Lookup existing seeded row by (submesh, token). Returns
        // row idx or -1.
        auto find_seeded = [&](const std::string &submesh,
                               std::uint16_t tok) -> int {
            const auto curCnt =
                count.load(std::memory_order_acquire);
            const auto upper = (curCnt < k_dyeSwatchesPerSlot)
                ? curCnt : k_dyeSwatchesPerSlot;
            for (std::size_t i = 0; i < upper; ++i)
            {
        ...
        // Allocate a fresh placeholder row with the given (submesh,
        // token) identity. All other fields are zeroed; caller fills
        // def_*/r/g/b/override_active after.
        auto alloc_placeholder = [&](const std::string &submesh,
                                     std::uint16_t tok) -> int {
            const auto raw =
            ...
            ovr.template_id       = 0;
            ovr.r = 0; ovr.g = 0; ovr.b = 0;
            ovr.def_r = 0; ovr.def_g = 0; ovr.def_b = 0;
            ovr.def_a = 0xFF;
        ...
        // Pass 1 -- palette: allocate one placeholder per
        // `(submesh, token)` from the captured-row palette. No
        // default colours are seeded; the engine fills `def_*` live
        // on its first matching write via
        // `capture_default_if_unset`. `override_active` is left
        // false; the override pass below promotes the specific rows
        // the user ticked.
        for (const auto &pe : palette)
        {
            ...
            const auto tok = TokenTable::token_id_for_name(
                pe.token_name.c_str());
            if (tok == 0) { ++skippedUnresolved; continue; }
            (void)alloc_placeholder(pe.submesh_name, tok);
        }

        // Pass 2 -- user overrides: each entry merges onto the
        // matching palette row by `(submesh, token)`. If no palette
        // row exists (an override was saved for a row that wasn't
        // in the palette), allocate a fresh placeholder.
        ...
        // Slot stays LOCKED after seeding -- promotion only updates
        // existing placeholders; unrelated submeshes still fall
        // through (no row created).
        g_postReinitLock[s].store(true, std::memory_order_release);
        g_reinitCaptureOpen[s].store(false, std::memory_order_release);
        ...
        if (seeded != 0 || skippedUnresolved != 0)
        {
            log.info(
                ...
                slot, palette.size(), mergedOverrides,
                orphanOverrides, skippedUnresolved);
        }
        return seeded;
    }
    ...
    void dump_slot(int slot) noexcept
    {
        ...
        // First pass: group rows by submesh_name so the dump is
        // tree-shaped (one block per region). std::map for stable
        // iteration order (alphabetical) without extra sort.
        //
        // Includes BOTH promoted rows (content_hash != 0) AND
        // placeholder rows seeded by populate_from_persisted
        // (content_hash == 0, but token_id + submesh_name set). The
        // placeholder marker shows up in the per-row state column
        // so the log clearly distinguishes "saved + waiting for
        // engine write" from "live and substituting".
        std::map<std::string, std::vector<std::size_t>> bySubmesh;
        for (std::size_t i = 0; i < upper; ++i)
        {
            const auto &e = g_table[s][i];
            const auto tok = e.token_id.load(std::memory_order_relaxed);
            if (tok == 0) continue;   // truly empty row
            const auto &ovr = dye_state()[s].swatches[i];
            std::string key = ovr.submesh_name[0]
                ? std::string(ovr.submesh_name)
                : std::string("(unnamed)");
            bySubmesh[key].push_back(i);
        }

        const bool slotEn = dye_state()[s].slot_enabled;
        const auto frozenFlag = State::swatch_frozen(slot).load(
            std::memory_order_acquire);
        const auto postLock = g_postReinitLock[s].load(
            std::memory_order_acquire);
        log.debug(
            "[swatch-dump] slot={} rows={}/{} unique_submeshes={} "
            "slot_enabled={} swatch_frozen={} post_reinit_lock={}",
            slot, upper, k_dyeSwatchesPerSlot, bySubmesh.size(),
            slotEn, frozenFlag, postLock);
        if (upper == 0) return;
        ...
        for (auto &kv : bySubmesh)
        {
            ...
            // Use the first row's template_id as the region's tpl
            // tag (rows in the same submesh share template_id; this
            // is observation, not invariant -- if it diverges the
            // first one is still useful as a hint).
            const auto firstTpl = g_table[s][rows.front()]
            ...
            for (auto idx : rows)
            {
                ...
                const auto tok = e.token_id.load(
                    std::memory_order_relaxed);
                const char *tokName =
                    TokenTable::token_label_for(tok);
                ...
                const char *state =
                      isPlaceholder         ? "PLACEHOLDER"      // seeded from JSON, awaiting promotion
                              ...
                              std::size_t keep_count) noexcept
    {
        ...
        for (std::size_t i = 0; i < upper; ++i)
        {
            ...
            for (std::size_t j = 0; j < keep_count; ++j)
                if (keep[j] == k) { in_keep = true; break; }
            if (in_keep)
            {
