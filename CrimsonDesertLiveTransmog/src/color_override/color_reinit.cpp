...
namespace Transmog::ColorOverride::Reinit
{
    namespace
    {
        // Per-pass timing. The debounced apply worker has ~100 ms
        // of latency, so 1500 ms leaves enough headroom for the
        // untick apply to drive the engine's natural-pipeline
        // real-armor restore before the retick tears it down again.
        ...
        struct SlotReinitState
        {
            ...
            // because the snapshot grows under a lock in tick() and
            // the UI may peek at it for diagnostics.
    ...
    bool start_slot_reinit_once(int slot) noexcept
    {
        ...
        if (!st.phase.compare_exchange_strong(
                ...
                std::memory_order_acq_rel))
            return false;
        st.mode.store(SlotReinitState::ModeReinit1Pass,
                      std::memory_order_release);
        st.pass.store(0, std::memory_order_release);
        {
            std::lock_guard<std::mutex> lk(g_mutex);
        ...
        SwatchTable::reinit_capture_open(slot).store(
            true, std::memory_order_release);
        DMK::Logger::get_instance().debug(
            "[color-reinit] slot={} START 1-pass target={:#06x}",
            slot, m.targetItemId);
        return true;
    }
    ...
    bool schedule_color_commit_retick(int slot) noexcept
    {
        ...
        if (!st.phase.compare_exchange_strong(
                expected, SlotReinitState::TeardownApply,
                std::memory_order_acq_rel))
            return st.mode.load(std::memory_order_acquire)
                   == SlotReinitState::ModeCommitRetick;
        st.mode.store(SlotReinitState::ModeCommitRetick,
                      std::memory_order_release);
        st.pass.store(0, std::memory_order_release);
        DMK::Logger::get_instance().debug(
    ...
    {
        if (!valid_slot(slot)) return false;
        return g_state[static_cast<std::size_t>(slot)].phase.load(
    ...
    bool is_color_commit_retick_active(int slot) noexcept
    {
        if (!valid_slot(slot)) return false;
        auto &st = g_state[static_cast<std::size_t>(slot)];
        return st.phase.load(std::memory_order_acquire)
                   != SlotReinitState::Idle
    ...
    bool any_slot_reinit_active() noexcept
    {
        for (std::size_t i = 0; i < k_slotCount; ++i)
            if (g_state[i].phase.load(std::memory_order_acquire)
                != SlotReinitState::Idle)
                return true;
        return false;
    }
    ...
    void cancel(int slot) noexcept
    {
        ...
        auto &st = g_state[static_cast<std::size_t>(slot)];
        st.phase.store(SlotReinitState::Idle,
                       std::memory_order_release);
        st.pass.store(0, std::memory_order_release);
        {
            std::lock_guard<std::mutex> lk(g_mutex);
    ...
    {
        if (!valid_slot(slot)) return;
        const auto last = g_lastTargetItemId[
            static_cast<std::size_t>(slot)].load(
            std::memory_order_acquire);
        if (newTargetItemId == 0)
        {
            // Untick / clear: reset tracking but DO NOT wipe -- user
            // override choices on tick->untick->re-tick of the same
            // item should survive.
            g_lastTargetItemId[static_cast<std::size_t>(slot)].store(
        ...
        g_lastTargetItemId[static_cast<std::size_t>(slot)].store(
            newTargetItemId, std::memory_order_release);
    }
    ...
    void tick() noexcept
    {
        ...
        for (int slot = 0; slot < static_cast<int>(k_slotCount); ++slot)
        {
            ...
            if (phase == SlotReinitState::TeardownApply)
            {
                ...
                m.active = false;
                // Bypass apply_single_slot_transmog's equality
                // early-out and any other "nothing to do" shortcuts.
                // The reinit's whole point is to force the engine
                // through a full tear-down + restore cycle, even if
                // the engine thinks state is unchanged.
                force_apply_pending()[static_cast<std::size_t>(slot)]
                    = true;
                DMK::Logger::get_instance().debug(
                    "[color-reinit] slot={} TeardownApply pass={} "
                    "(prev_target={:#06x}, m.active=false; engine "
                    "should tear down fake + restore real if any)",
                    slot, st.pass.load(std::memory_order_acquire),
                    prevTarget);
                ::Transmog::manual_apply_slot(
                    static_cast<std::size_t>(slot));
                st.deadline_ms.store(now + k_reinitTeardownMs,
                                     std::memory_order_release);
                st.phase.store(SlotReinitState::TeardownWait,
                               std::memory_order_release);
                continue;
            }
            if (phase == SlotReinitState::TeardownWait)
            {
                if (now < st.deadline_ms.load(std::memory_order_acquire))
                    continue;
                const auto lastApplied = last_applied_ids()[
                    static_cast<std::size_t>(slot)];
                DMK::Logger::get_instance().debug(
                    ...
                    "completed)",
                    slot, lastApplied);
                st.phase.store(SlotReinitState::CarrierApply,
                               std::memory_order_release);
                continue;
            }
            if (phase == SlotReinitState::CarrierApply)
            {
                ...
                ::Transmog::manual_apply_slot(
                    static_cast<std::size_t>(slot));
                st.deadline_ms.store(now + k_reinitCaptureMs,
                                     std::memory_order_release);
                st.phase.store(SlotReinitState::CarrierWait,
                               std::memory_order_release);
                continue;
            }
            if (phase == SlotReinitState::CarrierWait)
            {
                if (now < st.deadline_ms.load(std::memory_order_acquire))
                    continue;
                if (st.mode.load(std::memory_order_acquire)
                    == SlotReinitState::ModeCommitRetick)
                {
                    DMK::Logger::get_instance().debug(
                        "[color-reinit] slot={} commit-retick done",
                        slot);
                    st.pass.store(0, std::memory_order_release);
                    st.phase.store(SlotReinitState::Idle,
                                   std::memory_order_release);
                ...
                SwatchTable::SwatchIdentity buf[
                    ::Transmog::ColorOverride::k_dyeSwatchesPerSlot]{};
                const auto captured = SwatchTable::snapshot_active_identities(
                    slot, buf,
                    ::Transmog::ColorOverride::k_dyeSwatchesPerSlot);
                {
                    std::lock_guard<std::mutex> lk(g_mutex);
            ...
            {
                // Intersection over NON-empty passes only. Empty pass
                // means "no data" (publisher pipeline wedged that
                // cycle); treating ∅ as 0 wipes everything. Skip
                // empty passes instead.
                ...
                std::size_t nonEmpty = 0;
                {
                    ...
                    for (int p = 0; p < 3; ++p)
                    {
                        ...
                        if (!st.seen[p].empty()) ++nonEmpty;
                    }
                    // DIAG: log every identity each pass captured.
                    // Helps debug the intersection -- you should see
                    // up to 32 unique tuples in seen[0] and a subset
                    // in seen[1] / seen[2]. If sizes are inconsistent
                    // with the per-pass `captured` lines, something
                    // is racing on g_mutex.
                    for (int p = 0; p < 3; ++p)
                    {
                        DMK::Logger::get_instance().debug(
                            "[color-reinit] slot={} seen[{}] size={}",
                            slot, p, st.seen[p].size());
                        for (std::size_t i = 0; i < st.seen[p].size(); ++i)
                        {
                            ...
                            DMK::Logger::get_instance().debug(
                                ...
                                p + 1, i, k.hash, k.stable, k.tpl,
                                k.token);
                        }
                    }
                    int firstNonEmpty = -1;
                    for (int p = 0; p < 3; ++p)
                        if (!st.seen[p].empty()) { firstNonEmpty = p; break; }
                    if (firstNonEmpty >= 0)
                    {
                        for (const auto &a : st.seen[firstNonEmpty])
                        {
                            ...
                            {
                                if (p == firstNonEmpty) continue;
                                if (st.seen[p].empty()) continue;
                                bool found = false;
                                for (const auto &x : st.seen[p])
                                    if (x == a) { found = true; break; }
                                if (!found) in_all = false;
                            }
                            if (in_all) keep.push_back(a);
                        }
                    }
                    for (auto &v : st.seen) v.clear();
                }
                const auto kr = SwatchTable::apply_keep_set(
                    slot, keep.data(), keep.size());
                SwatchTable::post_reinit_lock(slot).store(
                    true, std::memory_order_release);
                SwatchTable::reinit_capture_open(slot).store(
                    false, std::memory_order_release);
                ...
                // any unrelated pending edits (e.g. user picks the
                // user might want to revert before saving), which
                // breaks the "pending until Save" contract the
                // other UI buttons honour.
                ...
                DMK::Logger::get_instance().debug(
                    ...
                    slot, keep.size(), kr.kept, kr.hidden,
                    passCounts[0], passCounts[1], passCounts[2],
                    nonEmpty);
                st.pass.store(0, std::memory_order_release);
                st.phase.store(SlotReinitState::Idle,
                               std::memory_order_release);
