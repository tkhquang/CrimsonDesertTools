...
//
// `sub_140F46680(u32* dest, const char* name, int, u32 sentinel)` is
// the engine's master name-interning function. It assigns (or looks
// up cached) u32 token ids for shader-property names. We hook it to
// capture every `(name, token_id)` pair so `token_label_for(tokenId)`
// and `token_id_for_name(name)` can resolve names that aren't in the
// AOB-discovered static slot tables.
//
// Patch-proof: AOB anchored on the function prologue. Tokens
// generated by ANY engine code path (not just the registrar tables
// our discovery walks) are captured.
...
{
    /// One-shot install. Returns true on successful hook install.
