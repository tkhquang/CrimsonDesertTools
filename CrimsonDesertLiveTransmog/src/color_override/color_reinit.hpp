...
// cycle once, then Finalize captures the identity tuples seen on that pass. Finalize's intersection degenerates to
// "keep everything captured this pass" when only one pass is populated, so no ghost pruning happens in single-pass
...
namespace Transmog::ColorOverride::Reinit
{
    ...
    /**
     * Cancel any in-flight reinit (e.g. when slot is wiped due to transmog target change). Resets to Idle without
     * finalize.
     */
