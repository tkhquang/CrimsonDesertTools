...
//
// Each row corresponds to ONE shader-property write the engine emits during a captured apply for an LT slot. A row's
// identity tuple is:
//
//   (content_hash, stable_id, template_id, token_id)
//
//   - content_hash = u32 at arec+0x40 (publisher writes the arec into matInst+0xA0; the hash is stable across pool
//     recycling).
...
//
// SwatchEntry holds the engine-default RGBA captured at row first fire (when override_active == false). SwatchOverride
// holds the UI-mutable state (user RGB, per-row override_active flag) and is a parallel array indexed by row index. The
// two arrays share a single row index per slot.
...
namespace Transmog::ColorOverride::SwatchTable
{
    ...
    struct SwatchEntry
    {
        ...
        // Engine default 4-byte property value at row first fire when override_active was false. Stored canonically as
        // R,G,B,A (the setter writes BGRA so we unpack on capture).
    ...
    /**
     * UI-mutable per-row override state. Parallel array shares the row index with SwatchEntry. UI thread writes; setter
     * thread reads (atomic loads only).
     *
     * Each entry carries the per-row identity (token_id / submesh_stable_id / template_id) plus the captured alpha so
     * the picker can label rows and group by region without a cross-array lookup.
     *
    ...
    struct SwatchOverride
    {
        ...
        // Captured from `SkinnedMeshMaterialWrapper +0x28 -> +0x18` at publisher-hook time. ASCIIZ in the format
        // `cd_phm_00_<region>_NNNN[_NN_NN]`. Empty when capture failed (cold pool / non-pac source / SEH bail). 40
        // chars covers the engine's inline string-wrapper capacity (32 bytes) with safety margin.
    ...
    /**
     * Setter-side: capture the engine-default RGBA into the row's SwatchEntry AND mirror to SwatchOverride for the UI.
    ...
    /**
     * Toggle a per-row override. While off, the setter falls through and the engine's natural value flows.
     */
    void set_override_active(int slot, std::size_t idx, bool active) noexcept;

    /**
     * Quick check: is any row's override_active flag set for the given slot? The setter uses this together with the
     * per-slot master-enable gate (`slot_enabled_get`) to bail before reading the full row.
     */
    bool any_override_active_in_slot(int slot) noexcept;

    /**
     * Find the slot whose user-saved placeholder matches the given (submesh_name, token_id), or -1 if none exists.
     *
     * Placeholder rows are created by `populate_from_persisted` from JSON: `content_hash == 0` + non-empty
    ...
    /**
     * Promote a placeholder row to a bound row by writing the live matInst's identity fields into it. Companion to
     * `find_placeholder_slot`: same search (placeholders with `content_hash == 0` matching `submesh_name + token_id`),
     * but on hit it stores `template_id` / `stable_id` / `content_hash` into the row's `SwatchEntry` atomics so the
     * picker UI shows real `tpl 0x%04X` values instead of `tpl 0x0000`.
     *
    ...
    /**
     * Per-slot master-enable gate. The UI binds an ImGui::Checkbox to the returned reference.
     */
    bool slot_enabled_get(int slot) noexcept;
    void slot_enabled_set(int slot, bool v) noexcept;

    /**
     * Detected swatch count for the slot (the in-use row count). The picker gates the master Dye checkbox and the "show
     * swatch UI" branch on this being > 0.
     */
    std::size_t detected_swatch_count(int slot) noexcept;

    /**
     * Wipe the slot's apply-window timers and carrier-set state without touching the swatch rows or user override
     * choices. The picker invokes this after the Reset Slot button so the next apply re-captures cleanly.
     */
    ...
    /**
     * True if the slot was wiped by `wipe_swatch_table_for_slot` (user-triggered Reset Slot) and has not been
     * repopulated since. Read by PresetManager::snapshot_live_swatches_into to distinguish a user-intentional empty
     * state from the token-resolution race where live can be empty but the JSON baseline must be preserved.
     */
     ...
     * refuses to add new rows but still hits existing ones for substitution.
     */
    ...
    /**
     * Convenience: identity tuple snapshot for currently-active rows. Used by the reinit state machine to capture
     * per-pass keys.
     */
    ...
    //
    // Match semantics on load: an entry is consumed when the live setter captures a row with the same `(submesh_name,
    ...
    /**
     * Pre-seed the slot's swatch table from the persisted palette and user-override lists. Two passes:
     *
     *   1. Walk `palette` and allocate one placeholder row per `(submesh, token)`. All rows start with `override_active
     *      = false` and no captured default; the engine fills `def_r/g/b` on its first matching write via
     *      `capture_default_if_unset`.
     *   2. Walk `overrides`. For each entry, find the row already created above (matching submesh+token) and set
    ...
    //       ...
    //
    // Tagged so log grep is easy. Default-off in shipping; calls happen at slot-apply-end, preset save/load, and wipe
    // events.
    void dump_slot(int slot) noexcept;

    /**
     * Dump every slot back-to-back. Used by preset save/load to snapshot the full picture in one log block.
     */
