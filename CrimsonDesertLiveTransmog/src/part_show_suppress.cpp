...
{
    // Indexed by the low 16 bits of the IndexedStringA hash the game passes to sub_14081DC20 in R8. 1 = suppress
    // (return 0 from the show handler), 0 = pass through. Mirrors EquipHide's s_hideLocked layout.
    ...
    // every game patch, so these are resolved at runtime via init_slot_hashes() from a prior
    // scan_indexed_string_table() pass.
    struct SlotHashes
    {
        std::size_t slot;
    ...
    // slot_metadata.hpp's `partShowHashKey` field. Only the 5 original armor slots have CD_* slot-suffix hashes in
    // IndexedStringA -- accessory and weapon slots store nullptr there and are filtered out below. (Replaces a local
    // 5-entry table; extending the TransmogSlot enum is now safe -- nullptr trailing entries are skipped explicitly.)

    // s_slotHashes stays sized to k_slotCount because callers index it by TransmogSlot value (which can be any of the
    // 20 entries). The 15 trailing slots have all-zero hashes and act as a no-op for suppression -- correct behavior
    // since accessories don't use this hash-driven path.
    ...
    void set_hash_suppressed(uint32_t partHash, bool suppressed) noexcept
    {
        ...
        s_suppressedHashes[idx].store(suppressed ? 1u : 0u, std::memory_order_relaxed);
    }
    ...
    // Diagnostic: remember which 16-bit hashes we've already logged so each unique hash shows up once per session. Uses
    // the same 0x10000-byte layout as s_suppressedHashes so lookups stay
    // O(1) and branch-free.
                                        ...
                                        __int64 a6, __int64 a7, __int64 a8, __int64 a9)
    {
        ...
        const bool suppress = enabled && partHash != 0 && s_suppressedHashes[idx].load(std::memory_order_relaxed) != 0;
