...
namespace Transmog
{
     ...
     *
     * The thread waits for the game world to load, then creates a transparent layered window with a D3D11 WARP device
     * and renders ImGui to an offscreen texture that is composited onto the screen via GDI UpdateLayeredWindow. No DXGI
     * swap chain is created, so this approach is compatible with any DX wrapper (OptiScaler, ReShade, SpecialK).
     *
     ...
     *
     * Debounced (300 ms cooldown) to prevent key-repeat and polling overlap from rapidly flipping the state.
     * Thread-safe.
     */
