...
namespace Transmog::DyeRecordInject
{
    ...
    /**
     * Dye-record vector geometry.
     *
     * The same {qword data, u32 count, u32 capacity} header sits at this offset on BOTH an auth-table entry and the
     * DyeCopier destination struct -- the two structures share the layout. It is published here so every consumer
     * reads one copy: the injector, the capture path and the socket override all walk the same vector, and it moves
     * with the auth-table entry geometry (see auth_table.hpp) on patch day.
     */
    ...
    /**
     * Fill one 16-byte ARMOR_MOD record. This is the single writer for the field layout documented on ChannelState
     * above -- callers must not open-code it, or the layout ends up stated in as many places as there are producers.
     */
