...
namespace Transmog::PartShowSuppress
{
     ...
     * scene-graph removal. Hooking it lets us stop stale real-helm frames from flashing through during glide exits,
     * landings, and effect spawns.
     *
     * Signature matches EquipHide's gliding_fix port (x64 __fastcall, 9 args).
     */
    using PartAddShowFn = __int64(__fastcall *)(__int64 a1, uint8_t a2, uint64_t partHashPtr, float blend, __int64 a5,
    ...
    /**
     * @brief Populate the suppression table from a slot category mask. Bit N = TransmogSlot::N should be suppressed.
     * @details Clears table first, then enables suppression for every part hash mapped to each set bit.
     */
     ...
     *
     * @details IndexedStringA buckets are volatile across game patches, so the slot hashes can't be hardcoded. This
     *          must be called before the first set_mask() / on_part_add_show() call. If the map is missing a slot name,
     *          that slot silently becomes unsuppressable for this session and a warning is logged.
     *
