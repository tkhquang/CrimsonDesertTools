...
//
// Bottom-of-Transmog-tab UI: action buttons row + status footer.
// Lives at draw_overlay_content depth 1; closures captured via the
// caller-supplied (pending, pendingSave, PresetManager) tuple.
...
void draw_status_footer()
{
    ...
    if (slot_populator_fn())
        ui_text_colored(ImVec4(0.4f, 1.0f, 0.4f, 1.0f),
                        "SlotPopulator: READY");
    else
        ui_text_colored(ImVec4(1.0f, 0.4f, 0.4f, 1.0f),
                        "SlotPopulator: UNAVAILABLE");
