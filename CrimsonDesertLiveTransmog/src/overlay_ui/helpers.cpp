...
//
// Single TU containing the variadic Text helpers (decl-in-header,
// def-in-cpp pattern to keep MSVC's inline-variadic COMDAT failure out
// of every consumer TU) and the small utility helpers that section
// files share.
    ...
    for (std::size_t i = 0; i + nlen <= hay.size(); ++i)
    {
        ...
        for (; j < nlen; ++j)
        {
            const char a = static_cast<char>(std::tolower(
                static_cast<unsigned char>(hay[i + j])));
...
// the transition.
//
// Clears UI/PWS selections for slots that had a body-mesh prefab pick
// and returns the per-slot mask of slots that were cleared. The caller
// is responsible for the post-apply_to_state lastIds reconciliation:
// for each cleared slot, if the new preset's carrier equals lastIds[i]
// the caller must zero lastIds[i] so the apply pass tears down the
// prior body-mesh fake (otherwise the natural-pipeline cleanup hook
// never fires and the swap stays visible). When carriers differ,
// lastIds is left intact so the regular tear_down_fake path runs.
...
bool dye_picker_compute_channel_gap_tip(
...
// committed to the game, or the per-slot force-apply flag is set after
// a re-pick on the same carrier id.
...
// to tint the Save button so unsaved work is visible.
//
