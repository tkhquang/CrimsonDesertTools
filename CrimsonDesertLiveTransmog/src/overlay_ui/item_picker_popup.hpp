...
//
// Search-filterable picker popup used by the per-slot loop. Handles item selection plus the cross-slot body-mesh prefab
// browser. When `autoApply` is true, hovering an item starts a debounce timer; once it expires the slot-scoped apply
...
namespace Transmog
{
    ...
    [[nodiscard]] bool draw_item_picker_popup(const char *popupId, SlotUIState &ui, Transmog::TransmogSlot slotCategory,
                                              std::uint16_t &targetItemId, bool autoApply, std::size_t slotIdx,
