...
//
// Per-slot dye + color-override popup.  Drives the merged dye-record
// picker (16 ARMOR_MOD rows + material picker + top action bar) and
// hosts the Color Override tab item that delegates to
// draw_color_override_tab_body.  Called once per slot from the
// per-slot loop in draw_overlay_content when the slot's dye chip is
// clicked (popup is owned here -- BeginPopup/EndPopup pair stays
...
// The caller manages the per-slot ImGui::PushID/PopID pair, so we
// just bail out -- non-armor slots (weapons, accessories) skip the
// dye chip entirely.
if (!isArmorSlot)
    return;
...
auto &dyeSlot =
    Transmog::ColorOverride::dye_state()[slot];
const std::size_t detected =
    Transmog::ColorOverride::SwatchTable::
        detected_swatch_count(static_cast<int>(slot));
...
// Constant row geometry: every slot reserves space for the
// dye chip + CO chip + reload icon even when inactive. Without
// this, switching from a preset with dye/CO active to one
// without makes the standalone window auto-shrink, and
// switching back forces it to expand again. Pinning the
// reserved width keeps the window stable across preset
// switches.
...
if (firstOv)
{
    ...
    const ImVec2 center(cur.x + chip_h * 0.5f,
                        cur.y + chip_h * 0.5f);
...
{
    // Reserve the CO chip + reload icon slots so the
    // row's footprint matches an active-CO row exactly.
    ImGui::Dummy(ImVec2(chip_h, chip_h));
    ImGui::SameLine(0.0f, 4.0f);
    ImGui::Dummy(ImVec2(chip_h, chip_h));
}
...
// open re-fires on still-empty slots.  Declared in
// overlay_ui/state.hpp so color_override.cpp can read
// it too.
...
if (ImGui::BeginPopup("##dye_picker"))
{
    ...
    ImGui::SameLine();
    if (ImGui::Button("Close", ImVec2(0, 0))) ImGui::CloseCurrentPopup();
