...
//
// Declared as inline namespace-scope variables (C++17) so every TU that
// includes this header observes the same storage.  Types live alongside
// state because the per-slot SlotUIState struct is consumed by multiple
// section files and exposing it here avoids forward-declaration churn.
//
// State naming uses the `s_` prefix to mark it as module-internal even
// though it lives in `namespace Transmog` (no anonymous namespace -- a
// single shared anonymous namespace across translation units is not
// possible; inline vars + Transmog scope provide equivalent linkage
// while making cross-TU access explicit).
...
// Session-only UI-scale multiplier applied on top of the init-time
// auto-DPI scale (see dx_overlay.cpp). 1.0 = no override; the user
// picks 1.25/1.5/etc. from the header combo.  Only used in the
// standalone overlay; ReShade has its own scaling.  Not persisted --
...
// R/G/B link toggle.  Each unique submesh stable_id has its own entry
// in the slot's regionUI map.
...
// One-shot auto-trigger for the per-slot Color Override reinit.  When
// the popup opens on a slot whose swatch grid is empty (detected == 0)
// and no reinit is currently running, we fire a single-pass reinit so
// the user does not have to click Re-init.  Re-armed each time the
// popup closes (set false in overlay_ui.cpp's popup-closed branch).
inline bool s_coAutoTriggered[k_slotCount] = {};

// Preset rename / new-character dialog scratch buffers.
inline char s_newCharName[64]{};
