...
namespace Transmog
{
     ...
     *
     * Resolves the item's descriptor from the (AOB-resolved, cached) iteminfo registry and walks its per-body variant
     * entry list -- the same authoritative structure `dump_itemmesh_tsv` uses. The result holds every distinct rig mesh
     * (e.g. Kliff_Mask -> {cd_phm_00_mask_00_0271_a, cd_phw_01_..., cd_pom_01_...}). Items with no variant list
     * (single-rig) fall back to their rule-chain primary body mesh, so the result is non-empty for any item that owns a
     * real mesh. Returns empty only when the registries are not resolved yet, or when the item has no resolvable mesh.
     * All reads are SEH-guarded. The call is safe from the game thread and from the UI thread.
     *
     ...
     * cross-references against the loader registry, the StringInfo registry, and a byte-scan of committed asset-bundle
     * memory, so every item is accounted for even when its prefab string lives outside the swap-catalog's
