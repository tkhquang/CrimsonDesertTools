 ...
 *
 * DetourModKit does not own the mod's own state. Hooks are caller-owned handles, so Transmog::shutdown() is the only
...
namespace
{
    DMK::Result<void> on_ready(DMK::Session &session)
    {
        ...
        if (!ready)
        {
            logger.error("Transmog initialization FAILED ({}). Mod will not function.", ready.error().message());
            return ready;
        }

        // Best-effort: the mod still works through hotkeys when the overlay cannot come up.
