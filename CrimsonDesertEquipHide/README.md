...
| **Chest/Legs flickering** | Hiding chest armor can cause pants/legs to flash during movement while the chest is hidden. | Set `CascadeFix = true` for best-effort flicker reduction while bare-chested (armor-to-armor swaps auto re-hide; equipping the chest from bare-chest can need a manual show-then-hide re-toggle). The fix covers the character you control. A bare-chested companion standing on screen can still flicker until you switch back to them. |
