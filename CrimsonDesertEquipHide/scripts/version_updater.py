#!/usr/bin/env python3
"""
CrimsonDesertEquipHide Version Updater
...
3. Updates version in CrimsonDesertEquipHide_Readme.txt
...
README_TXT = BASE_DIR / "build" / "template" / "CrimsonDesertEquipHide_Readme.txt"
...
def update_readme_txt(version):
    """Update version in CrimsonDesertEquipHide_Readme.txt using string operations instead of regex."""
    ...
    try:
        ...
        print(f"Error updating CrimsonDesertEquipHide_Readme.txt: {str(e)}")
        import traceback
        traceback.print_exc()
...
"""
    ...
    if not CHANGELOG_MD.exists():
        ...
        if "# Changelog" in content:
            if "All notable changes" in content:
                parts = content.split("All notable changes to the CrimsonDesertEquipHide mod will be documented in this file.")
                if len(parts) > 1:
    ...
    header_end_pos = content.find("All notable changes to the CrimsonDesertEquipHide mod will be documented in this file.")
    if header_end_pos != -1:
...
def main():
    parser = argparse.ArgumentParser(description="CrimsonDesertEquipHide Version Manager")
