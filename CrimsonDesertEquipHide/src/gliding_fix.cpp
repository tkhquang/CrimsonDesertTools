...
namespace EquipHide
{
                                        ...
                                        __int64 a6, __int64 a7, __int64 a8, __int64 a9)
    {
        if (flag_gliding_fix().load(std::memory_order_relaxed) && should_skip_part_add_show(partHashPtr))
            return 0;
        // Snapshot the trampoline. A stray callback fired between the shutdown's remove_hook() (which restores prologue
        // bytes and disables the SafetyHook detour) and the DLL unmap would observe a NULL trampoline; tail-calling
        // through it would crash before the loader can free the page. Safe-default return mirrors the early-out skip
        // path.
