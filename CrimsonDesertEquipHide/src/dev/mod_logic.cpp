...
extern "C"
{
    ...
    __declspec(dllexport) unsigned Init(const CdReloadInitRequest *request) noexcept
    {
        ...
        {
            append_loader_log("[EquipHide][DEV] Init rejected an invalid or stale reload request");
            return 0;
        }
        ...
        {
            append_loader_log("[EquipHide][DEV] Init rejected a foreign wheel-host table");
            return 0;
        }
        ...
        try
        {
            ...
            if (!opened)
            {
                append_loader_log("[EquipHide][DEV] Session::start failed");
                return 0;
            }
            ...
            {
                DMK::log().error("[DEV] Equip hide initialization FAILED ({})", ready.error().message());
                s_session.reset();
                return 0;
            }
        ...
        catch (...)
        {
            ...
            append_loader_log("[EquipHide][DEV] Init threw an exception");
            s_session.reset();
            return 0;
        }
    }
    ...
    __declspec(dllexport) unsigned Shutdown() noexcept
    {
        ...
        try
        {
            ...
            // Mod teardown first, while this module's code pages are still mapped. It joins the workers, removes every
            // hook, and reports whether each prologue was restored.
            s_hook_restore_failed = !EquipHide::shutdown();
            ...
            (void)std::snprintf(
                verdict,
                sizeof(verdict),
                "[EquipHide][DEV] unload verdict: wheel=%zu xinput_self=%zu xinput_targets=%zu total=%zu "
            ...
            {
                append_loader_log("[EquipHide][DEV] unload REFUSED: a hooked prologue was not restored");
                return 0;
            }
            if (total_pins != inert_pins)
            {
                append_loader_log(
                    "[EquipHide][DEV] unload REFUSED: a pin reason outside the documented-inert set "
        ...
        {
            append_loader_log("[EquipHide][DEV] Shutdown threw an exception; refusing unload");
            return 0;
        }
    }
} // extern "C"
