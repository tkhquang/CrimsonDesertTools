...
namespace EquipHide
{
     ...
     *
     * Each binding is wired via DMK::Config::register_press_combo, which fuses INI registration, default-combo parsing,
     * and the InputManager press registration into a single call. The returned InputBindingGuards are stashed in a
     * process-lifetime static vector so the cancellation flag survives until InputManager teardown.
     *
