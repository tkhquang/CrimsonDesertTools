...
namespace EquipHide
{
     ...
     * @details Prevents hidden parts from flashing during state transitions (e.g. gliding exit). This function bypasses
     *          the vis check and calls the show handler directly, so we intercept it to suppress hidden parts.
     */
    using PartAddShowFn = __int64(__fastcall *)(__int64, uint8_t, uint64_t, float, __int64, __int64, __int64, __int64,
                                                __int64);
